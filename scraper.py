...
SCANNER_VERSION = "6.1-lubiana-hotfix"
...
# Bezpośrednie strony postępowań, które muszą być sprawdzane niezależnie od
# budowy strony zbiorczej. To jest też mechanizm awaryjny dla serwisów WordPress,
# gdzie tytuł wpisu bywa umieszczony w znaczniku <header>.
DIRECT_NOTICE_URLS: dict[str, list[dict[str, str]]] = {
    "Ośrodek Hodowli Zarodowej Lubiana": [
        {
            "url": "https://ohzlubiana.pl/2026/07/15/zaproszenie-do-skladania-ofert-na-realizacje-inwestycji-budowlanej-w-formule-doprojektuj-i-wybuduj-pod-nazwa-rozbudowa-fermy-bydla-w-zakresie-budowy-dwoch-obor-wolnostanowi/",
            "fallback_title": "Rozbudowa fermy bydła – budowa dwóch obór wolnostanowiskowych wraz z budynkiem dojarni i szpitala, infrastrukturą towarzyszącą i zagospodarowaniem terenu",
...
def normalize_text(value: str) -> str:
    ...
    # NFKD nie zamienia polskiego „ł”, dlatego robimy to jawnie.
...
def direct_notice_candidates(session: requests.Session, source: Source) -> list[Candidate]:
    """Pobiera wskazane bezpośrednie strony postępowań jako niezależny fallback."""
    results: list[Candidate] = []
    for notice in DIRECT_NOTICE_URLS.get(source.name, []):
        direct_url = notice["url"]
        try:
            real_url, page_html = fetch(session, direct_url)
        except Exception as exc:
            print(f"OSTRZEŻENIE: bezpośredni wpis {source.name} niedostępny: {exc}", file=sys.stderr)
            continue
        if not page_html:
            continue

        page = BeautifulSoup(page_html, "lxml")
        for unwanted in page.find_all(["nav", "footer", "script", "style", "noscript", "aside"]):
            unwanted.decompose()

        heading = page.find("h1") or page.find("h2")
        title = clean_text(
            heading.get_text(" ", strip=True) if isinstance(heading, Tag) else notice["fallback_title"],
            420,
        )
        if not title or not meaningful_label(title):
            title = notice["fallback_title"]

        article = page.find("article") or page.find("main") or page.body or page
        context = clean_text(article.get_text(" ", strip=True), 1800)
        score, matched = score_candidate(title + " " + context, source.url)
        if score < 60:
            score, matched = score_candidate(notice["fallback_title"], source.url)
        if score < 60:
            continue

        parsed_deadline = extract_deadline(context)
        deadline = parsed_deadline or notice.get("deadline") or None
        published = notice.get("published_date") or None
...
def scan_source(session: requests.Session, source: Source) -> list[dict]:
    ...
    candidates: list[Candidate] = direct_notice_candidates(session, source)
...
def main() -> int:
    print(f"Wersja skanera: {SCANNER_VERSION}")
    sources = load_sources()
    existing = load_existing()
    session = requests.Session()
    session.headers.update({
        "User-Agent": "Mozilla/5.0 (compatible; AgroTenderMonitor/6.0; +https://github.com/)",
